...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("netm"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("netm://"))
    {
        uri.replace(0, 11, "netm:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "NetM.lnk";
}
...
{
    return GetAutostartDir() / "netm.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("NetM-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  netm-qt [" + tr("command-line options") + "]                     " + "\n";
