...
// Copyright (c) 2013-2014 NetM Developers
...
uint256 hashGenesisBlock("0xc7a6240e49d6e36ac3cc975bf1adab150a2d5df5ee0910cce09d111698ecb028");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // NetM: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "NetM Signed Message:\n";
...
{
    // NetM: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespanNEW = 60 ; // NTM: every 1 minute
static const int64 nTargetSpacing = 60; // NTM: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NetM: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // NetM: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x099981836ea7732ab9f3b75c8a85f6266d3ce695300a1a0a55d7302cd8695406");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Solar plane pacific crossing";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	...
	//block.nTime    = 1386325540;
        ...
        if (fTestNet)
        {
            block.nTime    = 1433027550;
            block.nNonce   = 1678382;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x8acd807b309f2d6c6e2e65a4b1a9819befc872ad6b99fc03e46788b0b3f24ebf"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb0, 0xb0, 0xb0, 0xb0 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NetMMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NetMMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NetMMiner(CWallet *pwallet)
{
    printf("NetMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("netm-miner");
    ...
    try { loop {
        ...
        printf("Running NetMMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NetMMiner, pwallet));
}
